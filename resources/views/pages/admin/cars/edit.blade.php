...
<div class="admin-form-container">
    <div class="form-header">
        <h1>Szerkesztés: {{ $car->model }}</h1>
        <a href="{{ route('admin.cars.list') }}" class="btn-back">← Vissza</a>
    </div>

    <form action="{{ route('admin.cars.update', $car->id) }}" method="POST" enctype="multipart/form-data" class="styled-form">
        @csrf
        ...
        <div class="form-grid">
            ...
            <div class="form-section">
                ...
                <select name="motor_type" id="motor_type">
                    @php
                        $types = ['benzin', 'Enyhe benzin', 'Plug-in hibrid', 'elektromos', 
                                  'Plug-in hibrid / benzinüzemű', 'Plug-in hibrid, benzinüzemű', 
                                  'Enyhe hibrid / Benzin', 'Hibrid / Benzin'];
                    @endphp
                    @foreach($types as $type)
                        <option value="{{ $type }}" {{ old('motor_type', $car->motor_type) == $type ? 'selected' : '' }}>
                            {{ $type }}
                        </option>
                    @endforeach
                </select>
                ...
                <div class="row" style="margin-top:15px;">
                    <div>
                        ...
                        <input type="number" name="motor_cc" id="motor_cc" value="{{ old('motor_cc', $car->motor_cc) }}" required>
                    </div>
                    <div>
                        <label for="horsepower">Lóerő</label>
                        <input type="number" name="horsepower" id="horsepower" value="{{ old('horsepower', $car->horsepower) }}" required>
                    </div>
                </div>
...
{{-- Styles are identical to the Create page --}}
@endsection